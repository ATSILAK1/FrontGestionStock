...
@Component({
  ...
  imports: [
    ...
    CommonModule,
    AutoCompleteModule,
    TransactionachatdisplayComponent,
    NavbarComponent,
  ],
...
export class TransactionachataddComponent implements OnInit {
  ...
  constructor(
    ...
    private transactionAchatService: TransactionService,
    private serviceFournisseur: FournisseurService
  ) {}

  // Dialog box Visibility variable
  isVisible = false;
  ...
  SubmitFrom() {
    ...
    transaction.informationTransaction =
      this.transactionForm.value.informationTransaction!;
    transaction.quantiteTransaction =
      this.transactionForm.value.quantiteProduit!;

    this.transactionAchatService
      .AjouterTransactionAchat(transaction)
      .subscribe({
        next: (value) => {
